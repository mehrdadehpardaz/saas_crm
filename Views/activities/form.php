...
// مسیر «بازگشت» / «انصراف»: فعالیت‌ها همیشه زیرمجموعه‌ی یک تسک‌اند،
// پس همیشه به صفحه‌ی همون تسک برمی‌گردیم؛ اگر هیچ تسکی در دسترس نبود
// (فعالیت مستقیماً به یک مشتری وصله، بدون تسک)، به لیست تسک‌ها می‌رویم.
...
<form method="POST" action="index.php?page=activities&action=<?= $is_edit ? 'update&id='.$id : 'create' ?><?= $back_task_id ? '&task_id='.(int)$back_task_id : '' ?>" class="af-form">
    ...
    <!-- فقط جهت اطلاع — پیگیری فعلی این تسک. برای تغییرش از فیلدهای
         «تاریخ/موضوع پیگیری بعدی» پایین همین فرم استفاده کن. -->
    <div class="af-task-box">
        <h4>⏰ پیگیری فعلی این تسک</h4>
        ...
        <div class="af-task-box-readonly">
            <?php if (!empty($task['next_followup_date'])): ?>
                ...
                <?php endif; ?>
            <?php else: ?>
                <div class="af-ro-empty">پیگیری بعدی‌ای برای این تسک تعیین نشده.</div>
            <?php endif; ?>
        </div>
